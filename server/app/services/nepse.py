...
@cache.ttl(seconds=LIVE)
async def get_floor_sheet_of(
    symbol: str,
    business_date: Optional[str] = None,
    size: int = 500,
) -> Any:
    """
    Returns floorsheet rows for a specific symbol.

    NEPSE does not expose a reliable per-symbol floorsheet endpoint
    (the company_floorsheet route returns 403). We use the general
    floorsheet endpoint and filter client-side — same approach the
    NEPSE website uses.
    """
    sym = symbol.upper()
