...
# Path to the WASM binary bundled in this repo
...
class TokenParser:
    ...
    def __init__(self) -> None:
        ...
        # 5 exported WASM functions — each computes one slice boundary
    ...
    def parse_token_response(
        ...
        # Positions to drop from accessToken (computed from salts in original order)
        ...
        # Positions to drop from refreshToken (salts swapped s1/s2)
        ...
        # Reconstruct by concatenating slices, skipping the injected characters
        parsed_access = (
...
class PayloadParser:
    ...
    NEPSE uses a lookup table (dummyData) combined with today's date and the
    salts to produce a per-request payload id that prevents simple replay attacks.

    which values:
      'stock-live'  — for live stock price endpoints
      'sector-live' — for sector index endpoints
      anything else — for general data endpoints
    """

    # Hardcoded lookup table extracted from the NEPSE JS bundle
    DUMMY_DATA = [
    ...
    def calculate_payload_id(
        ...
        """
        Args:
            given_id:      The `id` field from /api/nots/nepse-data/market-open
            token_details: The raw auth response dict (contains salt1..salt5)
            which:         One of 'stock-live', 'sector-live', or anything else

        Returns:
            The integer payload id to send in the request body as {"id": <value>}
        """
        ...
        # Salts stored as 0-indexed list matching basic-bgnr's implementation:
        # salts[0]=salt1, salts[1]=salt2, salts[2]=salt3, salts[3]=salt4, salts[4]=salt5
        salts = [
            token_details.get(f"salt{i}", 0) for i in range(1, 6)
        ]

        # For sector-live (floorsheet-style): e%10<5 → salts[3] (salt4), else salts[1] (salt2)
        # For general: e%10<5 → salts[3] (salt4), else salts[1] (salt2)
        # Both use the same index selection — matches basic-bgnr's getPOSTPayloadID
        if which == "sector-live":
            # FloorSheet variant: e%10<4 threshold
            idx = 1 if payload_id % 10 < 4 else 3
        else:
            # General variant: e%10<5 threshold
